...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=OPENVM_GUEST_GCC");
    println!("cargo:rerun-if-env-changed=OPENVM_GUEST_AR");
    ...
    // When building the OpenVM guest target, cc-rs will add `-march=rv64im -mabi=lp64`, but
    // the default host compiler may treat those as x86 flags unless we set a RISC-V target.
    if target == "riscv64im-unknown-openvm-elf" {
        // Respect user/toolchain-provided compiler overrides. For example, one could:
        //   export CC_riscv64im_unknown_openvm_elf=riscv64-unknown-elf-gcc
        //   export AR_riscv64im_unknown_openvm_elf=riscv64-unknown-elf-ar
        ...
        let ar_target_underscored = format!("AR_{}", target.replace('-', "_"));
        let user_specified_ar = var_os("AR").is_some()
            || var_os(&ar_target_underscored).is_some()
            || var_os(format!("AR_{}", target)).is_some();

        if !user_specified_cc {
            // Use GCC for the RISC-V guest build (no clang fallback), trying a small set of
            // common RV64-capable RISC-V GCC names.
            ...
            if !user_specified_ar {
                let mut archivers = Vec::new();
                if let Ok(v) = var("OPENVM_GUEST_AR") {
                    archivers.push(v);
                }
                if let Some(prefix) = gcc.strip_suffix("gcc") {
                    archivers.push(format!("{prefix}ar"));
                }
                if let Some(ar) = archivers
                    .into_iter()
                    .find(|ar| Command::new(ar).arg("--version").output().is_ok())
                {
                    build.archiver(ar);
                }
            }
